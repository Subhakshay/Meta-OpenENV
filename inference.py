...
def llm_agent(obs):
    if not HAS_LLM: return rule_based_agent(obs)
    parts = [f"Policy: {obs.get('active_policy_version','v1')}", f"Ticket: {obs.get('ticket_id','')}", f"Subject: {obs.get('subject','')}", f"Body:\n{obs.get('body','')}"]
    if obs.get("system_notice"): parts.append(f"\nSYSTEM NOTICE:\n{obs['system_notice']}")
    for f in ["sentiment_score","account_age_days","loyalty_score","lifetime_value","region","contract_type"]:
        if obs.get(f) is not None: parts.append(f"{f}: {obs[f]}")
    ws = obs.get("world_state_summary",{})
    parts.append(f"\nWorld: bal=${ws.get('company_balance',10000):.0f} churn={ws.get('churn_risk',0):.2f} sla={ws.get('sla_breaches',0)} diff={ws.get('difficulty_level',1)}")
    try:
        r = _client.chat.completions.create(model=MODEL_NAME, messages=[{"role":"system","content":_SYS},{"role":"user","content":"\n".join(parts)}], temperature=0.1, max_tokens=600)
...
def log_start(task, env_type, model): print(f"[START] task={task} env={env_type} model={model}", flush=True)
def log_step(s, a, r, d, cat=False): print(f"[STEP] step={s} action={a} reward={r:.2f} done={str(d).lower()} catastrophic={str(cat).lower()}", flush=True)
def log_end(ok, steps, score, rewards, cat=False): print(f"[END] success={str(ok).lower()} steps={steps} score={score:.3f} catastrophic={str(cat).lower()} rewards={','.join(f'{r:.2f}' for r in rewards)}", flush=True)

async def run_episode(task_id=2, env_type="gauntlet", attacker_enabled=True, agent_fn=None, model_name="Rule-Based"):
    if agent_fn is None: agent_fn = rule_based_agent
    ...
    if attacker_enabled:
        ...
        try:
            tickets = atk.generate_batch(n=12, difficulty_level=env.world_state.difficulty_level, defender_error_history=[], active_policy=env.policy_registry.get_active())
            env.set_attacker_tickets(tickets)
            obs = env._build_observation(tickets[0], None)
        except Exception as e:
            print(f"  [ATK ERROR] {e}"); atk = None

    ep_id = await db.create_episode(session_id=env.session_id, task_id=task_id, attacker_enabled=attacker_enabled, drift_enabled=True, difficulty_init=env.world_state.difficulty_level, env_type=env_type)
    ...
    while True:
        try:
            ...
            astr = "fallback"
        tkt = env._ticket_queue[min(env.world_state.tickets_processed, len(env._ticket_queue)-1)] if env._ticket_queue else {}
        result = env.step(action)
        rv = result["reward"]
        rewards.append(rv); steps += 1
        done = result["done"]
        ...
        await db.insert_step(episode_id=ep_id, step_number=steps, ticket_id=tkt.get("ticket_id",""), action=action, defender_reward=rv, attacker_fitness=af, breakdown=result.get("reward_breakdown",{}), policy_version=env.policy_registry.active_version_id(), was_post_drift=result.get("drift_notice") is not None, deception_strategy=strategy, template_index=tkt.get("template_index",-1))
