...
def plot_results():
    ...
    df_snapshots = pd.read_sql_query(
        "SELECT * FROM world_state_snapshots ORDER BY id", conn
    )
    ...
    df_episodes["reward_rolling"] = (
        df_episodes["mean_defender_reward"].rolling(window=10, min_periods=1).mean()
    )
    plt.plot(
        df_episodes.index,
        df_episodes["reward_rolling"],
        label="Defender Reward (10-ep rolling)",
    )
    plt.xlabel("Episode")
    ...
    plt.figure(figsize=(10, 5))
    df_episodes["drift_rolling"] = (
        df_episodes["drift_accuracy"].rolling(window=10, min_periods=1).mean()
    )
    plt.plot(
        df_episodes.index,
        df_episodes["drift_rolling"],
        color="orange",
        label="Drift Accuracy (10-ep rolling)",
    )
    plt.xlabel("Episode")
    plt.ylabel("Accuracy")
    ...
    stale_counts = df_snapshots.groupby("interval")["snapshot_json"].apply(
        lambda x: (
            x.apply(
                lambda s: pd.Series(json.loads(s)).get("stale_decisions_made", 0)
            ).max()
            if len(x) > 0
            else 0
        )
    ...
    plt.plot(
        df_episodes.index,
        df_episodes["attacker_win_rate_final"],
        color="purple",
        label="Attacker Win Rate (rolling 50-step window)",
    )
    ...
    balances = [
        json.loads(s).get("company_balance", 10000)
        for s in df_ep_snaps["snapshot_json"]
    ]
    plt.plot(
        range(len(balances)), balances, marker="o", label=f"Balance (Ep {sample_ep})"
    )
    plt.xlabel("Step")
